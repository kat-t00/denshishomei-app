...
const TemplateStore = (() => {
  ...
  function saveAll(templates) {
    try {
      ...
      // ただしプライベートブラウジング等で保存機能自体が制限されている場合も同じ例外系に
      // 落ちてくるため、「容量超過」と決め打ちせず、両方の可能性を案内した上で
      // 実際のエラー内容も添えて呼び出し元(app.js)に投げ直す(誤診断を防ぐため)
      console.error('テンプレートの保存に失敗しました', e);
      throw new Error('テンプレートの保存に失敗しました。' +
        'ブラウザの保存容量が上限に達しているか、プライベートブラウジング等で保存機能が制限されている可能性があります。' +
        '通常モードで開き直すか、「テンプレートをバックアップ」で保存後に使わなくなったテンプレートを削除してください。' +
        '\n（詳細: ' + (e.name || 'Error') + ' ' + (e.message || '') + '）');
    }
  }
